...
function verificaUsuarioJaCadastrado(email) {
  for (var i = 0; i < db_usuarios.usuarios.length; i++) {
    var usuario = db_usuarios.usuarios[i];
...
function addUser(nome, senha, email) {
  ...
  let usuarioJaExiste = verificaUsuarioJaCadastrado(email);
  ...
  if (!usuarioJaExiste) {
    ...
    renderizaAlert('Usuário salvo com sucesso! Proceda para o login.', 'success');
    setTimeout( function () {
      window.location.href = 'login.html';
    }, 1000);
  }
  else {
    renderizaAlert('Usuário já foi cadastrado.', 'warning');
  }
}
